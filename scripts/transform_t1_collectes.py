"""
Transform data from T1 Excel file in 'input' folder
into summary format matching 'COLLECTES DECHETERIES T1 2025.xlsx'

This script processes data from ALL déchetteries for T1 period (Janvier-Mars)
...
if __name__ == "__main__":
    print("="*70)
    print(" " * 15 + "TRANSFORMATION T1 (Premier semestre)")
    print("="*70)
    print("\nCe script transforme les données T1 en format COLLECTES")
    ...
    # Chercher le fichier T1 dans le dossier input
    input_file = None
        ...
        for file in os.listdir(input_dir):
            ...
            if os.path.isfile(file_path):
                ...
                if ext.lower() in excel_extensions:
                    ...
                    if 'T1' in file_upper and '25' in file_upper:
    ...
    if input_file is None or not os.path.exists(input_file):
        print(f"\n{'='*70}")
        print("ERREUR : Aucun fichier T1 trouve dans le dossier 'input' !")
        print(f"{'='*70}")
        print(f"\nLe dossier 'input' doit contenir un fichier Excel T1 (.xlsx ou .xls)")
        print(f"  Dossier recherche : {input_dir}")
        print(f"\nVÉRIFICATIONS :")
        print(f"  1. Y a-t-il un fichier Excel T1 dans le dossier 'input' ?")
        print(f"  2. Le fichier n'est-il pas ouvert dans Excel ?")
        print(f"  3. Le fichier contient-il 'T1' et '25' dans son nom ?")
        print(f"{'='*70}\n")
        sys.exit(1)
    
    # Nom du fichier de sortie
    output_file = os.path.join(output_dir, "COLLECTES DECHETERIES T1 2025.xlsx")
